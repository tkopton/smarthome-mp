        ...
        try:
            ...
            if status_code == 200:
                ...
                for obj in response_data:
                    logger.info("Device ID:" + obj["id"])
                    logger.info("Serial Number:" + obj["serialNumber"])
                    logger.info("Serial Number:" + obj["config"]["name"])
                    # creating object and adding it to the result set
                    device_obj = result.object(ADAPTER_KIND, "device", obj["config"]["name"])
